...
export class SessionWatcher {
  ...
  getStatus(fitWindowOverride) {
    ...
    // latch can never drift. Reason ordering: no_transcript → low_confidence → insufficient_data.
    // (metrics_unreliable RETIRED in v2.2 — confidence gate covers data quality.)
    ...
    // specific than the generic insufficient_data. An empty-but-present
    // file sets _transcriptSeen=true on open → falls through to insufficient_data (the distinction).
      ...
      confidence: baseline.confidence, postKneeGrowthCalls,
    ...
    return {
      ...
      metricsReliable: true, // hardcoded for API backward compat (v2.2: _metricsReliable retired from gate pipeline)
