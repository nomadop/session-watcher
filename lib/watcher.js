...
import { lStar, rho, phi, paybackP, timingWeight, regret, etaCalls, theilSen, nStar } from './metrics.js';
import { median } from './stats.js';
import { effectiveL } from './l-measure.js';
import { computeCalibrationGate, callIdentity, applyFrozen, makeLatchEntry, validateLatch } from './latch.js';
...
export class SessionWatcher {
  ...
  _baselineAndKavgLive(prefix) {
    ...
    const { kneeTurn, taskCtx, isRealKnee } = detectKnee(seq.length ? seq : [dead]);
    ...
    const baseline = { dead, task: taskCtx, total: dead + taskCtx, source, confidence, kneeTurn: segKnee, isRealKnee: isRealKnee === true };
  ...
  _baselineAndKavg(prefix, opts = {}) {
    ...
    if (!latchStore || !prefix.length) return { ...live, latched: false };
    const entry = this.ensureLatchForPrefix(prefix, latchStore);
    if (!entry) return { ...live, latched: false };
    const baseline = applyFrozen(entry);
    // kAvg denominator MUST use the FROZEN kneeTurn (spec §2.5) — else knee jitter re-enters via the
    // denominator and x* still fakes a jump.
    const apiCalls = Math.max(1, prefix.length - baseline.kneeTurn - 1);
    const kAvg = Math.max(0, (live.L - baseline.total) / apiCalls);
    return { baseline, L: live.L, kAvg, apiCalls, latched: true };
  }
  ...
  getStatus(fitWindowOverride) {
    ...
    const { isRealKnee, ...baselineOut } = baseline;

    // k_fit: Theil-Sen slope of the last fitWindow cacheRead values (current segment)
    const tail = seg.slice(-fitWindow).map(c => c.cacheRead);
    const kFitSlope = theilSen(tail);

    const Lstar = lStar(baseline.total, cRatio, kAvg);
    const LstarFit = kFitSlope > 0 ? lStar(baseline.total, cRatio, kFitSlope) : null;
    ...
    return {
      L, Lstar, LstarFit, Lcap, Lthreshold, restart, restartReason, calibratingReason,
      model,
      kAvg, kFitSlope, growth: seg.length ? Math.max(0, L - (seg.length >= 2 ? effectiveL(seg[seg.length - 2]) : L)) : 0,
      apiCalls, segment: this._segment,
      uptime: this._uptimeSec(),
      etaCalls: etaCalls(Lthreshold, L, kFitSlope),
