...
export async function startWatcher(env = process.env, { open = true, transcript, waitForPort = true, serverPath } = {}) {
  ...
  // Primary lookup by sessionId (transcript basename for statusline compatibility).
  // Fallback: CC sends different IDs to hook vs. statusline; the state file is keyed
  // by the transcript basename, but the hook's CLAUDE_CODE_SESSION_ID is per-restart.
  // Scan for a state file whose hookSessionId matches this session.
  ...
  // Stale state file from a dead server: clean up so the child can write a fresh one (prevents EEXIST).
  if (prev) {
  ...
  // Resolve server.js: prefer the bundled dist/server.js (self-contained with vendored deps),
  // fall back to source server.js (dev mode, needs project node_modules). Caller-supplied
  // serverPath always takes precedence (used by hooks via bundled dist/hooks/session-start.js).
  const defaultServerPath = existsSync(join(__dirname, 'server.js'))
    ? join(__dirname, 'server.js')       // bundled: dist/index.js → dist/server.js
    : join(__dirname, '..', 'server.js'); // source:  lib/launcher.js → ../server.js
...
export async function stopWatcher(env = process.env) {
  ...
  try { unlinkSync(stateFileFor(st.sessionId)); } catch {}
  return { stopped: false };
}
