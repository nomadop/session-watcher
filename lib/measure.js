...
import { TOOL_OVERHEAD, ALPHA_EMA, BETA_TREND, G_FLOOR } from './constants.js';
import { isSerenaError, parseSerenaFindSymbol, parseSerenaReferencing, parseSerenaPlainText } from './serena-parse.js';
...
export function parseBashFileRead(command) {
  ...
  if (m && !_hasShellExpansion(m[1])) {
    const filePath = m[1];
    // Check for || or pipes in the command (first ; segment, first line).
    const pipeType = _classifyPipe(cmd.split('\n')[0].split(';')[0], 'cat');
    if (pipeType === null) return null; // || or computation pipe → residual
...
// Split a shell command by real pipe operators (|), respecting quotes and escaped pipes (\|).
// Returns null if || (OR operator) is encountered — signals uncertain output.
function _splitByPipe(s) {
  ...
  while (i < s.length) {
    if (s[i] === '"') {
      ...
      // F9: count trailing consecutive backslashes in current to determine parity.
      // Even count → this \ escapes the pipe (no split). Odd → last \ pairs with this one (real pipe).
      let trailingBS = 0;
      for (let k = current.length - 1; k >= 0 && current[k] === '\\'; k--) trailingBS++;
      if (trailingBS % 2 === 1) {
        // Odd trailing backslashes: the last one in current pairs with this \, leaving | as a real pipe
        i++; // skip this backslash (it pairs with the previous one)
        const trimmed = current.trim();
        if (trimmed) stages.push(trimmed);
        current = '';
        i++; // consume the |
      } else {
        current += s[i++]; current += s[i++]; // even (or zero) trailing: this \ escapes the pipe
      }
    } else if (s[i] === '|' && i + 1 < s.length && s[i + 1] === '|') {
      return null; // || → OR operator, not a pipe
    } else if (s[i] === '|') {
      const trimmed = current.trim();
      if (trimmed) stages.push(trimmed);
      current = '';
...
// Classify a piped command to determine if the pipe preserves file structure.
// firstCmd: the first ;-segment of the command line.
// baseType: the type matched for the source command ('cat', 'head', 'grep-n').
// Returns the effective type ('cat', 'head', 'grep-n') if structure is preserved, or null for residual.
function _classifyPipe(firstCmd, baseType) {
  const allStages = _splitByPipe(firstCmd);
  if (allStages === null) return null; // || detected

  // allStages[0] = source command, allStages[1..] = pipe stages
  if (allStages.length < 2) return baseType; // no pipe — keep original type

  const pipeStages = allStages.slice(1);
  const pipeTools = pipeStages.map(s => s.trim().split(/\s+/)[0]);

  if (baseType === 'cat') {
    // cat file | head [| head]* → type:'head' (lineUpdate lines 1..N)
    if (pipeTools[0] === 'head' && pipeTools.slice(1).every(t => t === 'head')) return 'head';
    // cat file | grep -n [| head]* → type:'grep-n' (lineUpdate with line numbers)
    ...
    // Everything else: computation → residual
    return null;
  }

  if (baseType === 'head') {
    // head file | head [| head]* → still head
    if (pipeTools.every(t => t === 'head')) return 'head';
    return null;
  }

  if (baseType === 'grep-n') {
    // grep-n file | head [| head]* → still grep-n (head just truncates, N:content preserved)
...
function _hasShellExpansion(p) {
  if (p === '~' || p.startsWith('~/')) {
    // ~/path is deterministically resolvable (os.homedir()), but still check
    // the remainder for nested expansions ($(...), globs, backticks).
...
function _isUnresolvablePath(p) {
  if (p === '~' || p.startsWith('~/')) {
    // ~/path is deterministically resolvable — same carve-out as _hasShellExpansion.
    const rest = p.slice(1);
    if (/\$[({A-Za-z_]|`/.test(rest)) return true;
    if (/[*?]/.test(rest)) return true;
    return false;
...
// Shared computeUpdate for replace_symbol_body: returns editDelta with value=0
// (unknown old body; net-zero assumption, self-corrects on next Read).
function _serenaBodyWrite(input, result, _cwd, ctp) {
  if (isSerenaError(result)) return null;
  const body = input.body ?? '';
  if (!body) return null;
  const spent = charsToTokens(body, ctp) + TOOL_OVERHEAD.Serena;
...
export const BUILTIN_ADAPTERS = [
  ...
  // ─── Serena MCP read-like adapters ──────────────────────────────────────────
  {
    ...
    extractPath: (input, cwd) => {
      ...
      return null; // project-wide search → multi-file, handled in computeUpdate
    },
    computeUpdate: (input, result, cwd, ctp) => {
      if (isSerenaError(result)) return null;
      ...
      // Only track items that have body content (actual source code seen)
      const withBody = parsed.items.filter(item => item.body);
      if (withBody.length === 0) return null;

      // Single-file (relative_path in input) → lineUpdate
      if (input.relative_path) {
        const allLines = [];
        for (const item of withBody) {
          const lines = item.body.split('\n');
          for (let i = 0; i < lines.length; i++) {
            allLines.push([item.startLine + 1 + i, charsToTokens(lines[i], ctp)]);  // +1: Serena is 0-based, B is 1-based
          }
        }
      ...
      // Project-wide (no relative_path) → grepMultiFile
      const files = {};
      for (const item of withBody) {
        const canon = canonicalizePath(item.path, cwd);
        const lines = item.body.split('\n');
        const entries = lines.map((l, i) => [item.startLine + 1 + i, charsToTokens(l, ctp)]);  // +1: 0-based → 1-based
  ...
  },
  {
    ...
    extractPath: () => null, // always multi-file
    computeUpdate: (_input, result, cwd, ctp) => {
      if (isSerenaError(result)) return null;
      ...
      for (const [rawPath, entries] of Object.entries(parsed.files)) {
        ...
        for (const entry of entries) {
          if (entry.context) {
            ...
            for (let i = 0; i < lines.length; i++) {
              lineEntries.push([entry.startLine + 1 + i, charsToTokens(lines[i], ctp)]);  // +1: 0-based → 1-based
            }
          }
        }
...
// Legacy EMA step (default α follows ALPHA_EMA = 0.20, window ≈ 5 turns). Kept for callers with explicit alpha.
export function emaStep(prevG, residual, alpha = ALPHA_EMA) {
  return alpha * residual + (1 - alpha) * prevG;
}

// Holt double exponential smoothing. Level tracks residual base rate; trend tracks
// directional change. Regime tracking: when residual shifts sustainably, trend accelerates
// convergence (2.5 vs 3.7 frames). α=ALPHA_EMA (level), β=BETA_TREND (trend).
export function holtStep(prevLevel, prevTrend, residual, alpha = ALPHA_EMA, beta = BETA_TREND) {
  const level = alpha * residual + (1 - alpha) * (prevLevel + prevTrend);
...
// For piped cat/head commands that parseBashFileRead returns null for (residual),
// extract the pipe "actor" (first downstream tool) and the source file as detail.
// Returns { name, detail } or null if not a piped file-read going to residual.
function _pipeActorDisplay(cmd) {
  // Strip cd/fn preambles for matching
  let stripped = cmd.replace(/^(cd\s+\S+\s*&&\s*)+/g, '').replace(/^(fn\w+\s*&&\s*)+/g, '');
  const firstLine = stripped.split('\n')[0].split(';')[0];

  // Match cat or head as first stage
  ...
  if (allStages === null) return null; // || → let normal flow handle display
  if (allStages.length < 2) return null; // no pipe

  // The first pipe stage is the "actor"
  const actorStage = allStages[1].trim();
  const actorTool = actorStage.split(/\s+/)[0];

  // Only override display if this command would go to residual.
  // Check: does _classifyPipe return null for this?
  const baseType = catMatch ? 'cat' : 'head';
  const pipeResult = _classifyPipe(firstLine, baseType);
  if (pipeResult !== null) return null; // structure preserved → normal display

  // Residual: show actor as name, source file as detail
  const detail = filePath.length > 40 ? filePath.slice(-40) : filePath;
  const name = actorTool.length > 40 ? actorTool.slice(0, 40) : actorTool;
  return { name, detail };
}
...
export function bashFeature(command) {
  ...
  // For piped file-read commands (cat/head) that go to residual, show the pipe actor
  // instead of the source reader. "cat file | python3" → name="python3", detail="file"
