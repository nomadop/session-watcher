...
import { TOOL_OVERHEAD, ALPHA_EMA, G_DELTA_CAP, G_FLOOR } from './constants.js';
...
// g's smoothing step. β=0 by construction: the output is a convex combination of prevG and residual,
// so g can never leave [min, max] of its inputs — no trend momentum to overshoot a settling burst
// and drive g below the arrival rate it is tracking. `cap` bounds |Δg| per call, which suppresses the
// one-off spikes that a level filter alone would forecast forward; pass cap=0 for the unbounded EMA.
export function emaStep(prevG, residual, alpha = ALPHA_EMA, cap = G_DELTA_CAP) {
