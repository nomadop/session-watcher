...
// Lineage. It shares the lineage walk with the agent page and nothing below it: no Source is read, so
// there is no active-path ordinal to address a row by and no abandoned-identity judgement to make. A
// person gets every row that was stored; an agent's page gets every row still on the active path,
// addressed by its ordinal. The two answer different questions, so that divergence is the design rather
// than a gap. Its only import is the transient `S{k}` view of the lineage it was handed.
import { labelHistorySources } from './turn.js';
 ...
 * @param {Array<{ sessionId: string, handoffId: number|null }>} args.lineage
...
export function buildTurnBrowse({ store, lineage }) {
  ...
  sources.forEach((entry, i) => {
    ...
    const headline = i === 0
      ? rootHeadline(rows)
      : (store.getHandoff(sources[i - 1].handoffId)?.nextTask ?? '');
...
/**
 * The lineage's headlines alone, without the rows under them.
 *
 * It reduces the browse snapshot rather than recomputing anything: the drawer and a Handoff Delivery
 * show the same wayfinder, and a second spelling of the root's opening or of a predecessor's handed
 * task is what would let those two answers diverge. Section existence carries over with it, so a
 * session that stored no rows contributes no entry while the labels of the sessions after it stay put.
 *
 * @param {object} args - as buildTurnBrowse takes them
 * @returns {Array<{ label: string, headline: string }>} oldest → newest
 */
export function lineageHeadlines({ store, lineage }) {
  return buildTurnBrowse({ store, lineage })
    .sections.map(({ label, headline }) => ({ label, headline }));
}
