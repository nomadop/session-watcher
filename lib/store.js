...
class Store {
  ...
  sweep(maxAgeMs, { now = Date.now(), isLiveSession } = {}) {
    ...
    for (const { session_id } of expired) {
      ...
      this._db.exec('BEGIN IMMEDIATE');
      try {
        // Consume profile_snapshot (written by server.js fold-completion) before cascade delete
        ...
        this.archiveSession(session_id, {
          model: snap?.model || sessRow?.model || null, projectId: sessRow?.project_id || null,
        ...
        // Cascade delete (does NOT touch profile/profile_paths — those are archival)
