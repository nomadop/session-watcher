...
// while a delivery stamp — which touches none of them, and runs inside `deliverHandoffByToken`'s fail-closed
...
class Store {
  ...
  archiveSegmentTelemetry(sessionId, segment, artifact) {
    ...
    try {
      ...
      // Read status INSIDE the write txn — the sole anti-clobber guard. A concurrent process (or the live
      // owner racing a sweep) may have completed this (session,segment) between the caller deciding to
      // archive and now; refuse to clobber a completed segment. Closes the live-vs-sweep TOCTOU without a telemetry_generation
  ...
  findPendingHandoffsByProject(projectId, sessionId, { ttlMs = 7 * 86400000 } = {}) {
