...
export function poll(w) {
  ...
  for (const raw of complete.split('\n')) {
    if (!raw) continue;
    // Boundary path is only entered when the cheap precheck passes (spec §4.3 FOLD-PARSE-COST). A
    // non-boundary line short-circuits before JSON.parse for the boundary check (usage lines still parse
    // via their own `"usage"` prefilter below).
    if (boundaryPrecheck(raw)) {
      let entry; try { entry = JSON.parse(raw); } catch { /* fall through */ entry = null; }
      if (entry && isUserTurnBoundary(entry)) { w._pendingTurnBump = true; continue; }
    }
    if (!raw.includes('"usage"')) continue;      // cheap prefilter for the assistant/usage path
  ...
  return { newCalls, changed };   // NOTE: no per-poll w._turnSeq++ anymore (RV-C7)
}
