...
import { matchAdapter, extractToolResultText, holtStep, gEffective, bashFeature, mcpDisplay, charsToTokens, countsToTokens, CJK_RE } from './measure.js';
...
export function foldCall(w, u, stepMeta = { toolUseCount: 0, loadToken: null }) {
  ...
  if (w._prevL != null) {
    ...
    const prevLevel = w._g_ema == null ? residual : w._g_ema;
    const prevTrend = w._g_trend ?? 0;
    const holt = holtStep(prevLevel, prevTrend, residual);
    w._g_ema = holt.level;
    w._g_trend = holt.trend;
...
function resetFoldState(w, { bumpSegment = false, bumpFoldRev = true, clearCalls = true } = {}) {
  ...
  if (clearCalls || bumpSegment) {
    ...
    w._g_ema = null; w._g_trend = 0; w._prevB = 0; w._prevL = null; w._prevTotalStock = 0; w._ctpOvershoot = 0;
    w._bLagLedger = { total: 0, byPath: new Map() };
