...
// One value, two readers: the LOWER BOUND of foldCall's segment floor (the Math.max against
// SEGMENT_DROP_FRACTION — it binds only below a 400-token stock), and classifyMiss's
// stockPreserved epsilon, which stayed absolute.
export const SEGMENT_DROP_EPSILON = 100;
// A /clear or /compact replaces the whole conversation prefix, so the context stock falls by tens of
// percent; the dips this floor rejects are sub-1% shrinks from a smaller cache_creation on an
// unchanged prefix. Pinned by test/fold.segment-boundary.test.js.
export const SEGMENT_DROP_FRACTION = 0.25;
...
export const CTP_OVERSHOOT_WARN = 0.05;   // dashboard calibration hint when ctpOvershoot/L exceeds this.
                                           // §2.4c: ctpOvershoot now EXCLUDES cc→cr phase lag (deferred to
                                           // the _bLagLedger and retired as L catches up), so this hint
                                           // reflects genuine CTP drift only — the ~0.8% residual across the
                                           // corpus. NOTE: currently has NO live consumer (frontend doesn't
                                           // render it, GC drops ctp_overshoot_ratio on archival) — see
                                           // PLC-D1/D2 in the deferred-minors backlog. The 0.05 threshold and
                                           // the boundary telemetry write-back are only worth wiring up if a
                                           // consumer is built.
