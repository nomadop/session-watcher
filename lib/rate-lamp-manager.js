...
export function mergeLedgerIntoStatus(status, ledger, currentKey) {
  ...
  // WHY: br-family from LIVE B and g (spec section 2.2). getStatus/computeRateLampInstant already
  // populated dhat/mf/br/xSweet; re-derive the dashboard chart landmarks here for the aux gradient.
  // Use B_default (position basis) — same B that watcher.getStatus uses for x/dhat/mf/br, so
  // landmarks and current position live in the same coordinate system.
  ...
  if (B > 0 && cRatio > 0 && g > 0) {
    ...
    if (dhat > 0 && mf > 0) {
      // WHY: compute br here as well so /api/status callers who bypass computeRateLampInstant still see it.
    ...
    // WHY: xSweet = 1 + dhat — the minimum-regret operating point on the x axis.
    status.rateLamp.xSweet = status.rateLamp.xSweet ?? (1 + dhat);
    status.rateLamp.wallP = 1 + cRatio;
    status.rateLamp.lBase = B; // WHY: dashboard reads rl.lBase (now = B, the live baseline)

    // rentMeter: live render state for the dual-bar UI (spec invariant 10).
    // Overwrite the null-safe default set at function entry.
    ...
    // sweetRentRate = burnRate at xSweet: L = B·(1+dhat) ⟹ burnRate = (L−B)/(cRatio·B) = dhat/cRatio.
    // Exact (not a surrogate); reuses the same dhat/cRatio already on rateLamp for cross-surface consistency.
    const sweetRentRate = (Number.isFinite(dhat) && cRatio > 0) ? dhat / cRatio : null;
    // GPT #6 fallback: top-level burnRate → rateLamp.burnRate → null.
