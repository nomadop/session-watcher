...
// Post-v2.2 §1.1: per-turn ΔL EMA (in-memory only — NOT persisted to ledger JSON).
// Maps sessionId → { prevTurnSeq, prevTurnL, deltaLPerTurn }. Volatile: rebuilt from live traffic.
const _emaState = new Map();

function emaForSession(sessionId) {
  let ema = _emaState.get(sessionId);
  if (!ema) {
    ema = { prevTurnSeq: null, prevTurnL: null, deltaLPerTurn: null };
    _emaState.set(sessionId, ema);
  }
  return ema;
}

export function updateDeltaLPerTurn(ema, { watcherTurnSeq, L }) {
  if (!Number.isFinite(watcherTurnSeq) || !Number.isFinite(L)) return;
  // Same turn = strict no-op. Prevents same-turn polls from drifting prevTurnL upward.
  if (watcherTurnSeq === ema.prevTurnSeq) return;
  if (ema.prevTurnSeq !== null && ema.prevTurnL !== null) {
    const turnGap = watcherTurnSeq - ema.prevTurnSeq;
    if (turnGap > 0 && turnGap <= 3) {
      const rawDelta = L - ema.prevTurnL;
      if (rawDelta > 0) {
        const deltaPerTurn = rawDelta / turnGap;
        ema.deltaLPerTurn = ema.deltaLPerTurn == null
          ? deltaPerTurn
          : 0.3 * deltaPerTurn + 0.7 * ema.deltaLPerTurn;
      }
    }
  }
  ema.prevTurnSeq = watcherTurnSeq;
  ema.prevTurnL = L;
}
  ...
  if (status.rateLamp?.reliable && ledger && ledger.stateKey === currentKey) {
    ...
    if (lBase > 0 && kAvg > 0 && cRatio > 0) {
      ...
      status.rateLamp.targetL = computeTargetL({
        ...
        lCap: status.rateLamp.L_cap ?? status.contextWindow ?? 200000,
      });
    }
    ...
    // Post-v2.2: per-turn ΔL EMA (in-memory only)
    const ema = emaForSession(currentKey);
    updateDeltaLPerTurn(ema, { watcherTurnSeq: ledger.currentTurnSeq, L: status.rateLamp.L_read });
    status.rateLamp.deltaLPerTurn = ema.deltaLPerTurn;
  } else {
    status.rateLamp = status.rateLamp || {};
    status.rateLamp.deltaLPerTurn = null;
