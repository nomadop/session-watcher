...
import { landmarksFor, nucleus } from './landmarks.js';
import { computeMovableFrac, computeBr, xRightFromBr, xLeftFromBr, BR_AMBER, BR_RED } from './bill-regret.js';
  ...
  if (status.rateLamp?.reliable && ledger && ledger.stateKey === currentKey) {
    ...
    if (ledger.kStableFrozen > 0 && status.baseline?.total > 0) {
      ...
      const { xExit, L_exit_fullCarry } = deriveFrozenExit(
        status.rateLamp.C_RATIO, ledger.kStableFrozen, status.baseline.total);
      status.rateLamp.kStable = ledger.kStableFrozen;
      status.rateLamp.xExit = xExit;
      status.rateLamp.L_exit_fullCarry = L_exit_fullCarry;
      // inDeepWater is now set by the br-based block below (overrides the old threshold derivation)
    }
    ...
    // --- br-migration: mf + br + inDeepWater + geometric landmarks ---
    ...
    if (lBase > 0 && cRatio > 0) {
      ...
      // br-threshold x-coordinates for dashboard chart lines + aux gradient
      const mfVal = status.rateLamp.mf;
      if (dhat > 0 && mfVal > 0) {
        status.rateLamp.xBrAmberR = xRightFromBr(BR_AMBER, dhat, mfVal); // right arm br=10%
        status.rateLamp.xBrAmberL = xLeftFromBr(BR_AMBER, dhat, mfVal);  // left arm br=10%
        status.rateLamp.xBrRedR = xRightFromBr(BR_RED, dhat, mfVal);     // right arm br=25%
      }
      // xSweet = 1 + dhat (from frozen kStable axis); null when kStable not yet frozen
      status.rateLamp.xSweet = dhat != null ? 1 + dhat : null;
      // xEntry/xSweet from kAvg (for backward compat dashboard display)
      ...
      status.rateLamp.lBase = lBase; // dashboard (heroDiptych/depthAux/historyChart) reads rl.lBase
    }
...
// it (warn.js discards the POST response; with no OS notify this is the alert's only delivery channel).
...
function settleEndedTurnBoundary(l, { endedTurnSeq, status }) {
  ...
  // F4: derive inDeepWater from THIS boundary's lReadAtBoundary via br (replaces deepWaterDisplay latch).
