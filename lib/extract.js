import { C_RATIO_TABLE, DEFAULT_C_RATIO, CONTEXT_WINDOW_TABLE, DEFAULT_CONTEXT_WINDOW, CTP_TABLE, DEFAULT_CTP } from './constants.js';
...
// Model-prefix → Two-CTP constants (spec §2.1). Resolved once per session from the first usage row's
// model field. Prefix match (not regex) — real ids are 'claude-opus-4-8', 'deepseek-v4-flash', etc.
// Uncalibrated models fall back to DEFAULT_CTP (conservative middle ground between Claude and DeepSeek).
export function ctpForModel(modelId = '') {
...
export function isUserTurnBoundary(entry) {
  ...
  // isMeta entries are harness-injected skill content (sourceToolUseID links them back to the
  // Skill tool_use) — NOT human turns. Treating them as boundaries would spuriously bump turnSeq.
  ...
  const c = msg.content;
  if (typeof c === 'string') {
    // A task-notification is the harness returning a sub-agent result — NOT human input.
    // Distinguisher: always STARTS with the XML tag (after optional whitespace).
