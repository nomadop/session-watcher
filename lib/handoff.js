...
export function createHandoffComposition({
   ...
   *           projectRoot: string|null, symbolRangesFor: function }} request
  ...
  function composePrepared({ input, measurement, filePaths, ctp, projectRoot, symbolRangesFor }) {
    ...
    const previousStats = {
      b_full: m.B, b_default: bDefault, g: m.g, mf: m.mf, br_exit: m.br,
      pp_exit: computePp(m.x, m.dhat), turns: measurement.turnSeq, total_l: m.L,
      dead, session_floor: sessionFloor, residual: Math.max(0, m.L - m.B),
    };
    // The gate parameters recomputed with the KEPT bucket as the B basis, for post-hoc analysis of the
    // keep/discard decision. The session floor rather than `dead` alone is the better predictor of the next
    // segment's baseline: it already carries the always-present session overhead.
