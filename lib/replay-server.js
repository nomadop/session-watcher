...
import { resolveClaudeCodeCacheTtl } from './harness/claude-code/cache-ttl.js';
import { initStore, closeStoreGlobal, getStore } from './store.js';
...
export async function startReplayServer({ transcriptPath, speed = 20, port = 0 }) {
  ...
  // One read of this process's declaration, handed to the owner composition and to the application that
  // reports its price, so the pair cannot name different lifetimes.
  const cacheTtl = resolveClaudeCodeCacheTtl();

  // The shared application, through the host's own composition root. The live driver is irrelevant here —
  // Transcript Playback installs its OWN application and driver pair inside the replay route — but the owner
  // still needs one, and this is the same factory every other host path uses.
  const watcher = createWatcherComposition({
    sessionId: 'replay', sourceLocator: transcriptPath, projectId: null, projectRoot: process.cwd(),
    stateDir: null, store: getStore(), isIgnored: null, cacheTtl,
  });
