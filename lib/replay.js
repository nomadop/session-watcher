...
export function indexTranscript(filePath) {
  ...
  while (pos < buf.length) {
    ...
    const head = buf.slice(pos, Math.min(pos + 512, lineEnd)).toString('utf8');
...
export class ReplayController {
  ...
  _scheduleNext() {
    ...
    // Trapezoidal integration of burnRate for rentMeter
    ...
    if (this._cursor < this._index.length) {
      ...
      // Minimum 500ms per step — gives dashboard time to render each frame visibly.
      // At speed=4 with 143 steps: total replay ≈ 143 * 0.5s = ~72s minimum.
      const delay = Math.max(500, clampedGap / this._speed);
