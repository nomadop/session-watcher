...
// Ensures the array exists, appends, trims to RECENT_STOP_EVENTS_LIMIT. The manager's advance loop is what
// pushes a fired event through it, on the reader path.
...
// sample: { seq, reliable, unavailableReason?, burnRate, L_read, turnSeq }. L_read is the step's measured
// L, which the Engine resolves to total stock on a detected miss and to cache read otherwise
// (test/measurement-engine.contract.test.js `miss L is total stock`) — the field name is the guard: a
// cacheRead-named sample is a bug (fails Task 2.5 validator).
export function applyFoldedCallSample(prev, sample) {
  ...
  // change the step's measured L — pinned by Task 0's FU-B1-coupling value test), so this fires only on an
  ...
  // `currentTurnSeq` is a consumer cursor the manager assigns from the frame after EVERY frame, zero-sample
