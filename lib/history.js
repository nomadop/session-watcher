...
export function computeHistoryPoint(w, c, _arr, _lockedModel, _fitWindow, _latchStore) {
  ...
  // totalStock = true context volume this row; next row's cacheRead converges to it.
  // Fixes: (1) parallel-tool turns showing stale cacheRead plateaus, (2) DeepSeek miss
  // where cacheCreation=0 makes effectiveL under-report actual stock.
  const L = c.cacheRead + c.cacheCreation + c.input;
  const x = B > 0 ? L / B : 1;
  return {
    ts: c.ts, segment: c.segment, L, B, x,
...
export function getHistory(w, fitWindowOverride) {
  ...
  if (canReuse) {
    ...
    // the first call folded into that segment, matching how foldCall locks _segmentModel — NOT
