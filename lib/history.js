...
import { lStar, phi, paybackP } from './metrics.js';
...
// `c` (identical to what today's loop passes to _baselineAndKavg) and `lockedModel` = the segment's
// creation model. A point is a PURE function of (arr, lockedModel), so computing it once and caching
// it is output-identical to recomputing it — this purity is exactly what makes the getHistory
// memoization safe. `fitWindow` is retained in the signature only for call-site stability with
// getHistory (the memo cache key) — ER-2 (Task 10) retired the kFit chain that was its ONLY consumer
// here, so it no longer affects the emitted point.
