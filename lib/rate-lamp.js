...
// #11 (fix wave): the xExit → L_exit_fullCarry derivation, extracted from the two sites that
// duplicated it (mergeLedgerIntoStatus in the manager, and computeRateLampInstant below). Pure; wraps
// computeXExitFromKStable as its nucleus.
// NOTE: inDeepWater is now br-based (br >= BR_AMBER), computed in mergeLedgerIntoStatus.
export function deriveFrozenExit(cRatio, kStable, lBase) {
...
export function computeRateLampInstant(snap, { scenario }) {
  ...
  const { xExit, L_exit_fullCarry } = deriveFrozenExit(cRatio, kStable, lBase); // #11: shared derivation
