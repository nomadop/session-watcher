 ...
 * @param {Array}  currentPoints - data points for the visible segment ({ L, g, ... }[])
 * @param {number|null} lastGEma - per-call EMA growth rate (tokens/call); null or <= 0 triggers g fallback
...
export function computeCrosshairLabel(snappedTurn, currentPoints, lastGEma) {
  ...
  const slope = (lastGEma > 0)
    ? lastGEma
    : (currentPoints[lastDataTurn - 1]?.g > 0 ? currentPoints[lastDataTurn - 1].g : 0);
