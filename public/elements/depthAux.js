...
export function mount(root, _ctx) {
  ...
  function renderBar(snapshot) {
    ...
    // computeLandmarkPositions expects {minX, maxX} shape; map from viewport.overviewDomain
    const overviewDomain = { minX: viewport.overviewDomain.min, maxX: viewport.overviewDomain.max };
    const positions = computeLandmarkPositions({ domain: overviewDomain, xEntry, xSweet, xExit, wallP, x });

    // Gradient (zones mode only — Pareto removed)
    gradientEl.style.background = buildZonesGradient(
      positions.entryPct, positions.sweetPct, positions.exitPct, positions.wallPct
    );
    ...
    ticksEl.innerHTML = buildLabelsHTML(
      positions.entryPct, positions.sweetPct, positions.exitPct, positions.wallPct, barWidth
    );

    // Marker + flag (x only, no burn rate — spec §3.5)
