...
// Unit tests for the Stop-hook thin client (hooks/warn.sh).
// warn.sh is a ZERO-INJECTION hook: it must ALWAYS exit 0 with EMPTY stdout (Stop-hook stdout would
...
const HOOK = join(__dirname, '..', 'hooks', 'warn.sh');
...
function makeSandbox({ curlBody = '{"notify":false}' } = {}) {
  ...
  // If warn.sh ever regressed into an OS-notification path, this would capture the call. v2.1 forbids it.
...
// Run the REAL hook via `bash warn.sh` (mirrors the registered `command: bash, args: [warn.sh]`),
...
test('warn.sh (69): server unreachable / dead port → silent exit 0, empty stdout', async () => {
...
test('warn.sh (68): sid + port come from stdin/statefile, not env; POST built correctly; stdout empty', async () => {
  const sb = makeSandbox();
  try {
...
test('warn.sh (R2-D1): notify:true response still triggers NO osascript/notify-send', async () => {
...
test('warn.sh (traversal): metachar / .. sids → exit 0, no curl, no fs touch, empty stdout', async () => {
